...
def prepare_env(args, argv):
    ...
    cfg['seen_label_path'] = join('data', args.data, 'label_seen.txt')
    cfg['unseen_label_path'] = join('data', args.data, 'label_unseen.txt')
    cfg['prompt_template_path'] = join('data', 'prompt_templates.txt')
    check_exists(cfg['train_path'])
    check_exists(cfg['test_path'])
    check_exists(cfg['seen_label_path'])
    check_exists(cfg['unseen_label_path'])
    check_exists(cfg['prompt_template_path'])
    cfg['num_seen_labels'] = len([t.strip() for t in open(cfg['seen_label_path'])])
    cfg['num_unseen_labels'] = len([t.strip() for t in open(cfg['unseen_label_path'])])
    
    # cache
    cfg['train_text_feat_path'] = join('data', args.data, 'train_text_features.npy')
    cfg['test_text_feat_path'] = join('data', args.data, 'test_text_features.npy')
...
def get_args(parse=True):
    ...
    parser.add_argument('--model', type=str, default='vke')
    ...
    parser.add_argument('--mode', type=str, default='GZSL', choices=['none', 'ZSL', 'GZSL'])
    ...
    parser.add_argument('--rmcls', action='store_true')
